...
public class GenesysVoicePlatform8 extends VoicePlatform {
	...
	public String postProcessInitialVariable(String name, String originalValue) {
		...
		} else if ("gvpTestData".equals(name)) {
			try {
				byte[] valToPrint = originalValue.getBytes("UTF-8");
				System.out.println("Verifying character array");
				for (char c : originalValue.toCharArray()) {
					System.out.println(c);
				}
				System.out.println("Escaping array of string");
				for (char c : originalValue.toCharArray()) {
					System.out.println("\\u"
							+ Integer.toHexString(c | 0x10000).substring(1));
				}
				System.out.println("Just trying to print the string");
				System.out.println(valToPrint); // TODO cleanup
	...
	protected IDocument renderInitialDocument(ILinkFactory links,
		...
		form.addVariable(new Variable("gvpTestData",
				"session.com.genesyslab.userdata.vh_unicode"));
		...
		for (int i = 0; i < parameterNames.length; ++i)
			nextLink.setParameters(parameterNames[i],
					initialCommand.getParameterValues(parameterNames[i]));
		...
		fields[fields.length - 3] = "gvpTestData";
		...
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i],
					initialCommand.getParameterValues(parameterNames[i]));
		...
		if (events.contains(cpaPrefix)) {
			...
			for (String event : events) {
				if (event.startsWith(cpaPrefix))
					...
					for (int i = 0; i < parameterNames.length; ++i)
						eventLink.setParameters(parameterNames[i],
								initialCommand
			...
			for (int i = 0; i < parameterNames.length; ++i)
				cpaLink.setParameters(parameterNames[i],
						initialCommand.getParameterValues(parameterNames[i]));
	...
	protected IDocument renderMetaDataRequest(ILinkFactory links,
		...
		for (int i = 0; i < params.length; i++) {
			createNextLink.setParameters(params[i],
					metaDataMessageRequest.getParameterValues(params[i]));
		}
	...
	protected IDocument renderMetaDataMessage(ILinkFactory links,
		...
		for (int i = 0; i < params.length; i++) {
			createNextLink.setParameters(params[i],
					metaDataMessageCommand.getParameterValues(params[i]));
		}
		block.addAction(send);
		if (isCtiC) {
			Receive receive = new Receive();
		...
		try {
			...
			if (userData.containsKey("content")) {
				...
				for (String kvpString : contentArray) {
					...
					dataMap.put(kvpArray[0],
							URLDecoder.decode((String) kvpArray[1]));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
